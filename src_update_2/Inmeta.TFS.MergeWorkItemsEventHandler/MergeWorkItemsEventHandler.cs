	...
	public class MergeWorkItemsEventHandler : ISubscriber
	{
		...
		public EventNotificationStatus ProcessEvent(IVssRequestContext requestContext, NotificationType notificationType, object notificationEventArgs, out int statusCode, out string statusMessage, out ExceptionPropertyCollection properties)
		{
			...
			try
			{
				...
				{
					var checkinNotification = (CheckinNotification) notificationEventArgs;

					if (ShouldMergeItemsIfNecessary(checkinNotification))
					{
                        TfsTeamProjectCollection impersonatedCollection = requestContext.GetImpersonatedCollection(checkinNotification.ChangesetOwner.UniqueName);

					    if (impersonatedCollection != null)
					    {
					        MergeWorkItems(impersonatedCollection, checkinNotification.Changeset);
					    }
					}
				}
			}
		...
		private bool ShouldMergeItemsIfNecessary(CheckinNotification checkinNotification)
		{
		...
		private void MergeWorkItems(TfsTeamProjectCollection tpc, int changesetId)
		{
			...
			var changesForChangeset = versionControlServer.GetChangesForChangeset(changesetId, false, 2147483647, null, null, true);
